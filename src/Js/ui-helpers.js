...
export const updateCollectionCount = (collectionSet = loadCollection()) => {
    ...
    if (desktopCounter)
        desktopCounter.textContent = String(collectionSize);
    if (mobileCounter)
        mobileCounter.textContent = String(collectionSize);
};
export const initializeSidebar = () => {
    ...
    if (!sidebar || !hamburgerMenuButton || !sidebarBackdrop)
        return;
...
export const initializeBrowseDropdown = () => {
    ...
    window.addEventListener("click", (event) => {
        ...
        if (!event.target.closest(".browse-dropdown")) {
            dropdownMenu.classList.remove("show");
        }
    });
};
export const getUniqueGenres = (booksArray) => {
...
export const populateGenreFilters = ({ genreList, desktopGenreList, mobileGenreList, includeAllOption = false, }) => {
    if (!desktopGenreList || !mobileGenreList)
        return;
...
export const renderBookCard = ({ book, originalIndex, inCollection, }) => {
    ...
    <div class="product-card" data-index="${originalIndex}">
      ${isNewBook ? `
      <!-- Edit/Delete Overlay (top of card) -->
